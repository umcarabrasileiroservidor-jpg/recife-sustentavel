  ...
  try {
    ...
    if (type === 'dashboard') {
      ...
      return res.status(200).json({
        stats: { activeUsers: u.rows[0].count, activeBins: b.rows[0].count, activeRewards: r.rows[0].count, activePenalties: p.rows[0].count, totalDiscards: d.rows[0].count },
        charts
      });
    }

    // 2. RELATÓRIOS (O que estava dando erro "Tipo não encontrado")
    if (type === 'reports') {
    ...
    // 3. AUDITORIA (O que estava dando 404)
    if (type === 'auditoria') {
      if (req.method === 'GET') { 
    ...
    // 5. BINS
    if (type === 'bins') {
      if (req.method === 'GET') { const { rows } = await pool.query("SELECT * FROM pontos_coleta ORDER BY criado_em DESC"); return res.status(200).json(rows.map((b: any) => ({ ...b, location: b.nome_local, type: b.tipos_aceitos?.[0] || 'geral', capacity: 100 }))); }
    ...
    // 6. REWARDS
    if (type === 'rewards') {
      if (req.method === 'GET') { const { rows } = await pool.query("SELECT * FROM recompensas ORDER BY criado_em DESC"); return res.json(rows.map((r:any) => ({...r, value: r.custo_pontos, partner: r.parceiro}))); }
    ...
    // 7. PENALTIES
    if (type === 'penalties') {
      if (req.method === 'GET') { const { rows } = await pool.query("SELECT p.*, u.nome as user, u.email FROM penalidades p JOIN usuarios u ON p.usuario_id = u.id ORDER BY p.data_aplicacao DESC"); return res.json(rows); }
